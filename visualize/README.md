...
3. 在合适的位置嵌入 **dataviewjs 代码块**:
   - **静态图 / 时序 / 状态机 / 架构 / 路径** → dataviewjs 块里 `innerHTML` 注入 SVG
   - **交互** → dataviewjs 块里用 JS 动态构建 DOM、绑事件
4. 写到 `C:\Users\34434\OneDrive\Obsidian\学习笔记\~Codex\` 下

> 为什么所有视觉块都走 dataviewjs?因为 codex 直接写到 markdown 里的内联 `<svg>` 标签在 Obsidian 阅读视图里渲染不可靠,用 dataviewjs 注入 DOM 才稳定。代价是需要装 Dataview 插件,但一劳永逸。

只有当你明说"生成一个 html""给我一个单独的 svg 文件"时,才会切换到桌面输出独立文件——那种情况下是纯原生 SVG / HTML,不需要 dataviewjs 包裹。
...
Obsidian 里打开 **Settings → Community plugins**,搜索 **Dataview** 装上并启用。然后在 **Dataview 设置**里打开 **Enable JavaScript Queries**(这一步必做,否则 dataviewjs 块只会显示为源代码,图和交互都看不到)。
...
- ✅ 在合适位置嵌入了 dataviewjs 代码块(里面是 SVG 或交互 UI)
- ✅ 打开笔记切到**阅读视图**,能看到渲染好的图或交互
...
1. **文件路径**:Windows 原生 shell 和 WSL 看到的路径不一样,SKILL.md 里有说明,codex 会先探测环境。路径中间有反斜杠:`学习笔记\~Codex` 是两个目录不是连写的名字。
2. **Dataview 插件是必须的**:所有视觉块(不论静态 SVG 还是交互)都用 dataviewjs 渲染。没装插件或没开 JavaScript Queries 的话,代码块只会显示源代码。
...
5. **博客发布预留**:如果以后要把笔记发布到博客,博客端需要加一个 dataviewjs 代码块的解析模块(或者把 dataviewjs 块里的 SVG 内容抽出来静态化)。当前设计优先 Obsidian 内阅读体验。
...
- **统一走 dataviewjs**——放弃内联 SVG 的"零依赖"幻觉,换来 Obsidian 阅读视图下稳定渲染

希望这套 skill 能让 codex 在你的 obsidian 工作流里变成一个真正能打的学习助手。

---

*skill 版本:v1.1 · 2026-04-17*
