---
name: visualize
description: 在 Codex CLI 里为学习/知识梳理类问答自动生成视觉化笔记 — 包括流程图、架构图、时序图、对比表、状态机、学习路径、交互卡片等。默认产出 Obsidian 兼容的 markdown 笔记(内嵌 SVG 或 dataviewjs 块),只有在用户明确要求单文件时才输出独立 .html / .svg。**务必在用户讨论概念、学习路线、系统架构、流程步骤、对比分析、数据关系等场景时主动触发本 skill**,哪怕用户没有明说"画一下""可视化""做个图"——只要内容具备空间结构、时序流程、分类对比或可交互演示特征,视觉化就能显著提升笔记质量。凡是涉及 Obsidian 学习笔记、知识卡片、概念图、学习路径、架构梳理、流程整理、状态转移、知识对比等用户意图时,都要优先考虑使用本 skill。
---
...
| 类型 | 用什么 | 理由 |
|------|--------|------|
| 静态图(流程/架构/时序/对比/学习路径) | **内联 `<svg>`** 直接写到 markdown | Obsidian 阅读视图原生渲染,零依赖,文件自包含 |
| 需要交互(点击展开、参数调节、动态过滤) | **dataviewjs 代码块** | 动态生成 DOM,能做状态管理 |
| 表格型数据视图(任务清单、笔记索引) | **dataview 查询块**(非 JS) | 简单声明式,性能好 |

混合使用没问题——一篇笔记里可以既有内联 svg、又有 dataviewjs 块。
