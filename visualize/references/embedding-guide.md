...
| 视图 | 快捷键 | 会渲染 |
|------|--------|--------|
| 阅读视图(Reading View) | `Ctrl+E` 切换 | 完整 markdown 渲染,包括内联 SVG、mermaid、HTML `<details>` 等 |
...
**什么时候选 mermaid 而不是 SVG?**
- 结构复杂(很多连线)但样式要求不高
- 需要让读者在源码里也能看懂
- 时序图、状态图、类图、甘特图

**什么时候选 SVG 而不是 mermaid?**
- 样式风格重要(pastel 色块、圆角 pill)
- 需要自定义布局(卡片式)
- mermaid 默认样式不满意
