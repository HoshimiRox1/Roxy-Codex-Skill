...
> ⚠️ **重要前提 1**:这里列的 7 种模式是**参考词汇**,不是**唯一可选**。看到内容先去 `design-thinking.md` 里分析结构本质,再决定用这里的模板、还是改造/组合它们、还是原创一个形式。
>
> 真实内容的多样性远远超过 7 种模板。如果你每次都机械地套这里的某一个,那你就是做题家,不是得意门生。

> ⚠️ **重要前提 2**:下面所有模板给的是**纯 SVG 代码**,方便展示结构和细节。但**实际写进 Obsidian 笔记时,必须用 dataviewjs 代码块包裹**:
>
> ````markdown
> ```dataviewjs
> const container = dv.el("div", "", { cls: "viz-svg" });
> container.innerHTML = `
>   <svg viewBox="..." xmlns="http://www.w3.org/2000/svg" style="...">
>     <!-- 这里放下面模板里的 SVG 内容 -->
>   </svg>
> `;
> ```
> ````
>
> **不允许**在 .md 里直接写裸 `<svg>` 标签。详见 `SKILL.md` 的"SVG 嵌入规范"。
>
> (单独输出到桌面的 `.svg` 文件例外——那是独立文件,不经过 markdown。)

遇到一类需求时,先看 `design-thinking.md` 理解内容的结构本质,然后:
1. 如果本质匹配这里某个模板 → 复制下来改内容(记得套上 dataviewjs 包裹)
2. 如果本质接近但有差异 → 把模板当起点,改造结构(记得套上 dataviewjs 包裹)
3. 如果本质完全不在这 7 种里 → 翻 `design-thinking.md` 的"设计模式库",参考那些没给 SVG 但列了思路的模式,自己搭(记得套上 dataviewjs 包裹)
...
不确定时,**不要先想哪个模板最省事**,而是回到 `design-thinking.md` 重新判断内容的结构本质与最佳展示方式。

如果几种形式都能成立,按这个顺序取舍:
1. 哪种形式让读者一眼更容易看懂
2. 哪种形式更符合内容本身的结构,而不是硬套模板
3. 哪种形式在当前交付介质里展示效果最好(Obsidian 用 dataviewjs / SVG,非 Obsidian 可用独立 HTML)
